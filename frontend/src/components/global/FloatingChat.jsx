...
const CHIPS = [
  ...
  "Who dominates death overs?",
  "Show RCB batting weakness",
];
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`fixed bottom-20 right-4 z-50 w-[min(100vw-2rem,360px)] rounded-xl border shadow-2xl bg-ipl-panel/95 border-white/10`}
          >
              ...
              {messages.map((m, i) => (
                <div
                  ...
                  className={`text-xs rounded-lg px-2.5 py-2 max-w-[90%] ${
                    m.role === "user" ? "ml-auto bg-ipl-orange/90 text-white" : "bg-white/5 text-gray-300"
                  }`}
                >
                  {m.text}
                </div>
            ...
            <form
              className="flex gap-2 p-3 border-t border-white/6"
              onSubmit={(e) => {
                e.preventDefault();
                send(input);
              }}
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="fixed bottom-4 right-4 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-ipl-cyan to-ipl-orange text-white shadow-lg"
